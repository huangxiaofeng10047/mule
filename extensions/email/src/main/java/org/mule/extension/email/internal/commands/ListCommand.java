...
public final class ListCommand {
    ...
    try {
      ...
      for (Message m : folder.getMessages()) {
        ...
        if (matcher.test(attributes)) {
          ...
          list.add(MuleMessage.builder().payload(emailContent).attributes(attributes).build());
        }
      }
      return list;
  ...
  private Object readContent(Message m) {
    Object emailContent;
    EmailContentProcessor processor = EmailContentProcessor.getInstance(m);
    String body = processor.getBody();
    List<MuleMessage> parts = new ArrayList<>();
    List<MuleMessage> attachments = processor.getAttachments();
